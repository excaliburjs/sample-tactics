...
for(r in O)"number"==typeof(n=O[r])&&(o=t[e].axes[n])!==this._oldPads[e].getAxes(n)&&(this.at(e).updateAxes(n,o),this.at(e).events.emit("axis",new t$(n,o,this.at(e))));this._oldPads[e]=this._clonePad(t[e])}}}/**
...
else{let s=this.l<.5?this.l*(1+this.s):this.l+this.s-this.l*this.s,r=2*this.l-s;t=ev.hue2rgb(r,s,this.h+1/3),e=ev.hue2rgb(r,s,this.h),i=ev.hue2rgb(r,s,this.h-1/3)}return new ef(255*t,255*e,255*i,this.a)}toString(){let t=this.h.toFixed(0),e=this.s.toFixed(0),i=this.l.toFixed(0),s=this.a.toFixed(0);return`hsla(${t}, ${e}, ${i}, ${s})`}}(h=$||($={})).None="None",h.Top="Top",h.Bottom="Bottom",h.Left="Left",h.Right="Right",(l=$||($={})).getOpposite=/**
 ...
 * @param milliseconds
 * @param clock
 */function eT(t,e){var i;let s=new ex,r=null!==(i=null==e?void 0:e.schedule.bind(e))&&void 0!==i?i:setTimeout;return r(()=>{s.resolve()},t),s.promise}(d=G||(G={}))[d.X=12]="X",d[d.Y=13]="Y";/**
 ...
 */class eN{constructor(){/**
     ...
     * @param text
     * @param pos
     */write(t,e,i){this._imageSource.isLoaded()&&this._spriteFont.render(t,e,null,i.x,i.y)}}class eW{constructor(t,e){this._gl=t,this._texture=e}use(){let t=this._gl;t.activeTexture(t.TEXTURE0),t.bindTexture(t.TEXTURE_2D,this._texture)}disable(){let t=this._gl;t.bindTexture(t.TEXTURE_2D,null)}}class e${constructor(t){this.width=t.width,this.height=t.height,this._gl=t.gl,this._setupFramebuffer()}setResolution(t,e){let i=this._gl;this.width=t,this.height=e,i.bindTexture(i.TEXTURE_2D,this._frameTexture),i.texImage2D(i.TEXTURE_2D,0,i.RGBA,this.width,this.height,0,i.RGBA,i.UNSIGNED_BYTE,null)}get frameBuffer(){return this._frameBuffer}get frameTexture(){return this._frameTexture}_setupFramebuffer(){// Allocates frame buffer
...
this.register(new e1),this.register(new e7),this.register(new e2),this.register(new e5),this.register(new eQ),this.register(new ej),this._screenRenderer=new eJ(t),this._renderTarget=new e$({gl:t,width:t.canvas.width,height:t.canvas.height}),this._postProcessTargets=[new e$({gl:t,width:t.canvas.width,height:t.canvas.height}),new e$({gl:t,width:t.canvas.width,height:t.canvas.height})]}register(t){this._renderers.set(t.type,t),t.initialize(this.__gl,this)}get(t){return this._renderers.get(t)}_isCurrentRenderer(t){return!this._currentRenderer||this._currentRenderer===t}beginDrawLifecycle(){this._isDrawLifecycle=!0}endDrawLifecycle(){this._isDrawLifecycle=!1}draw(t,...e){this._isDrawLifecycle||this._alreadyWarnedDrawLifecycle||(this._logger.warn(`Attempting to draw outside the the drawing lifecycle (preDraw/postDraw) is not supported and is a source of bugs/errors.
 ...
 *
 *
 */class sm{constructor(t,e){this._stopped=!1,this._repeatBuilder=e,this._repeatContext=new sR(t),this._actionQueue=this._repeatContext.getQueue(),this._repeatBuilder(this._repeatContext)}update(t){this._stopped||(this._actionQueue.isComplete()&&(this._actionQueue.clearActions(),this._repeatBuilder(this._repeatContext)),this._actionQueue.update(t))}isComplete(){return this._stopped}stop(){this._stopped=!0,this._actionQueue.clearActions()}reset(){}}class sf{constructor(t,e,i,s){if(this._started=!1,this._stopped=!1,this._entity=t,this._tx=t.get(iN),this._motion=t.get(iW),this._speed=s,this._offset=new eg(e,i),s<=0)throw t7.getInstance().error("Attempted to moveBy with speed less than or equal to zero : "+s),Error("Speed must be greater than 0 pixels per second")}update(t){this._started||(this._started=!0,this._start=new eg(this._tx.pos.x,this._tx.pos.y),this._end=this._start.add(this._offset),this._distance=this._offset.size,this._dir=this._end.sub(this._start).normalize()),this.isComplete(this._entity)?(this._tx.pos=em(this._end.x,this._end.y),this._motion.vel=em(0,0)):this._motion.vel=this._dir.scale(this._speed)}isComplete(t){let e=t.get(iN);return this._stopped||e.pos.distance(this._start)>=this._distance}stop(){this._motion.vel=em(0,0),this._stopped=!0}reset(){this._started=!1,this._stopped=!1}}class sv{constructor(t,e,i,s){this.entity=t,this._started=!1,this._stopped=!1,this._tx=t.get(iN),this._motion=t.get(iW),this._end=new eg(e,i),this._speed=s}update(t){this._started||(this._started=!0,this._start=new eg(this._tx.pos.x,this._tx.pos.y),this._distance=this._start.distance(this._end),this._dir=this._end.sub(this._start).normalize());let e=this._dir.scale(this._speed);this._motion.vel=em(e.x,e.y),this.isComplete(this.entity)&&(this._tx.pos=em(this._end.x,this._end.y),this._motion.vel=em(0,0))}isComplete(t){let e=t.get(iN);return this._stopped||new eg(e.pos.x,e.pos.y).distance(this._start)>=this._distance}stop(){this._motion.vel=em(0,0),this._stopped=!0}reset(){this._started=!1,this._stopped=!1}}/**
 ...
 *
 */class sW extends iz{constructor(t,e=!0){super(),this.draw=t,this.useTransform=e,this.type="ex.debuggraphics"}}let s$={PreUpdate:"preupdate",PostUpdate:"postupdate",PreDraw:"predraw",PostDraw:"postdraw"};/**
...
rr._POS=new eg(0,0),rr._SCALE=new eg(1,1),rr._ACC=new eg(0,0),rr._VEL=new eg(0,0),rr._VEL_ACC=new eg(0,0),rr._SCALE_FACTOR=new eg(0,0);class rn extends s5{constructor(){super(...arguments),this.types=["ex.transform","ex.motion"],this.systemType=to.Update,this.priority=-1}update(t,e){let i,s;for(let r=0;r<t.length;r++){i=t[r].get(iN),s=t[r].get(iW);let n=t[r].get(i8);if(null==n?void 0:n.sleeping)continue;let o=s.acc.clone();(null==n?void 0:n.collisionType)===X.Active&&(null==n?void 0:n.useGravity)&&o.addEqual(iB.gravity),null==n||n.captureOldTransform(),// Update transform and motion based on Euler linear algebra
...
e.cancel();continue}let t=$.fromDirection(e.mtv),i=e.mtv.negate(),s=e.colliderA.worldPos.squareDistance(e.colliderB.worldPos);this.distanceMap.set(e.id,s),// Publish collision events on both participants
e.colliderA.events.emit("precollision",new tq(e.colliderA,e.colliderB,t,i)),e.colliderB.events.emit("precollision",new tq(e.colliderB,e.colliderA,$.getOpposite(t),i.negate()))}}postSolve(t){var e,i;for(let s of t){if(s.isCanceled())continue;let t=s.colliderA,r=s.colliderB,n=null===(e=t.owner)||void 0===e?void 0:e.get(i8),o=null===(i=r.owner)||void 0===i?void 0:i.get(i8);if(n&&o&&(n.collisionType===X.Passive||o.collisionType===X.Passive))continue;let a=$.fromDirection(s.mtv),h=s.mtv.negate();// Publish collision events on both participants
s.colliderA.events.emit("postcollision",new tX(s.colliderA,s.colliderB,a,h)),s.colliderB.events.emit("postcollision",new tX(s.colliderB,s.colliderA,$.getOpposite(a),h.negate()))}}solvePosition(t){var e,i;// if bounds no longer intersect skip to the next
...
let s=$.fromDirection(e.mtv);e.colliderA.events.emit("postcollision",new tX(e.colliderA,e.colliderB,s,e.mtv)),e.colliderA.events.emit("aftercollisionresolve",new tj(e.colliderA,e.colliderB,s,e.mtv,e)),e.colliderB.events.emit("postcollision",new tX(e.colliderB,e.colliderA,$.getOpposite(s),e.mtv.negate())),e.colliderB.events.emit("aftercollisionresolve",new tj(e.colliderB,e.colliderA,$.getOpposite(s),e.mtv.negate(),e))}for(let e of(// Store contacts
 ...
 */class r${constructor(t){this.inputs=t,this._handlers=new Map}/**
...
let n=t&&t.pointerScope===W.Document?document:this.canvas;this.input={keyboard:new es,pointers:new rU(n,this),gamepads:new t6},this.input.keyboard.init({grabWindowFocus:null===(i=null===(e=this._originalOptions)||void 0===e?void 0:e.grabWindowFocus)||void 0===i||i}),this.input.pointers.init({grabWindowFocus:null===(r=null===(s=this._originalOptions)||void 0===s?void 0:s.grabWindowFocus)||void 0===r||r}),this.input.gamepads.init(),this.inputMapper=new r$(this.input),// Issue #385 make use of the visibility api
...
if(this.selectionManager.resetHighlight(),"move"===this.selectionManager.currentSelectionMode){let e=this.selectionManager.findMovementRange(this.selectionManager.currentUnitSelection);this.selectionManager.showHighlight(e,"range");let i=this.board.getCellByWorldPos(t.worldPos);if(i){let t=this.selectionManager.findPath(i,e);this.selectionManager.showHighlight(t,"path")}// attack
     ...
     * @param cell 
     */async highlightUnitRange(t){if(t?.unit){let e=t.unit,i=this.board.pathFinder.getRange(t.pathNode,~e.player.mask,e.unitConfig.movement+1);this.selectionManager.showHighlight(i,"attack");let s=this.selectionManager.findMovementRange(e);this.selectionManager.showHighlight(s,"range")}else this.selectionManager.reset()}async maybeSelectUnit(t){// check if the cell clicked has a unit, then select it
 ...
 * SPDX-License-Identifier: BSD-3-Clause
 */const eO="important",eH=" !"+eO,eN=eL(class extends ez{constructor(t){if(super(t),t.type!==eU.ATTRIBUTE||"style"!==t.name||t.strings?.length>2)throw Error("The `styleMap` directive must be used in the `style` attribute and must be the only part in the attribute.")}render(t){return Object.keys(t).reduce((e,i)=>{let s=t[i];return null==s?e:e+`${i=i.includes("-")?i:i.replace(/(?:^(webkit|moz|ms|o)|)(?=[A-Z])/g,"-$&").toLowerCase()}:${s};`},"")}update(t,[e]){let{style:i}=t.element;if(void 0===this.ut)return this.ut=new Set(Object.keys(e)),this.render(e);for(let t of this.ut)null==e[t]&&(this.ut.delete(t),t.includes("-")?i.removeProperty(t):i[t]=null);for(let t in e){let s=e[t];if(null!=s){this.ut.add(t);let e="string"==typeof s&&s.endsWith(eH);t.includes("-")||e?i.setProperty(t,e?s.slice(0,-11):s,e?eO:""):i[t]=s}}return ep}}),eW=eL(class extends ez{constructor(t){if(super(t),t.type!==eU.ATTRIBUTE||"class"!==t.name||t.strings?.length>2)throw Error("`classMap()` can only be used in the `class` attribute and must be the only part in the attribute.")}render(t){return" "+Object.keys(t).filter(e=>t[e]).join(" ")+" "}update(t,[e]){if(void 0===this.it){for(let i in this.it=new Set,void 0!==t.strings&&(this.st=new Set(t.strings.join(" ").split(/\s/).filter(t=>""!==t))),e)e[i]&&!this.st?.has(i)&&this.it.add(i);return this.render(e)}let i=t.element.classList;for(let t of this.it)t in e||(i.remove(t),this.it.delete(t));for(let t in e){let s=!!e[t];s===this.it.has(t)||this.st?.has(t)||(s?(i.add(t),this.it.add(t)):(i.remove(t),this.it.delete(t)))}return ep}});let e$=class extends eM{static #t=this.styles=tL`
        @keyframes fadeIn {
            from {
                opacity: 0;
    ...
    `;firstUpdated(){this.menuHtml?.addEventListener("animationend",t=>{"fadeOut"===t.animationName&&(this._show=!1,this.menuHtml?.classList.remove("hide"),this.requestUpdate())})}sendEvent(t){return()=>{this.dispatchEvent(new Event(t)),this.hide(),this.requestUpdate()}}show(){this._show=!0,this._debounce=Date.now()}hide(){let t=Date.now();t-this._debounce>200&&(this.menuHtml?.classList.add("hide"),this.clearEvents())}render(){let t=this._show?eu`<div class="overlay" @click=${this.hide}></div>`:e_;return eu`
        ${t}
        <div class=${eW({menu:!0,show:this._show})} style=${eN({left:`${this.left}px`,top:`${this.top}px`})}>
...
// }
this.selectionManger.reset()}}return!0}}class eX extends l{constructor(t){super({pos:t,coordPlane:g.Screen,vel:z(U(-30,-100),0),width:100,height:100})}onInitialize(t){this.cloudSprite=Q.CloudSheet.toSprite(),this.cloudSprite.scale=tp,this.graphics.use(this.cloudSprite)}onPostUpdate(t,e){this.pos.x+this.cloudSprite.width<0&&(this.pos.x=t.screen.contentArea.right+this.cloudSprite.width)}}const eZ=new y({width:800,height:800,displayMode:m.FitScreenAndFill,antialiasing:!1,suppressHiDPIScaling:!0,configurePerformanceCanvas2DFallback:{allow:!1}}),eK=new class extends I{onInitialize(t){t.input.pointers.primary.once("down",()=>{t.goToScene("level1")}),this.add(new eX(z(800,0))),this.add(new eX(z(400,300))),this.add(new eX(z(700,700))),this.title=new l({pos:z(400,400),coordPlane:g.Screen}),this.title.scale=tp,this.title.graphics.use(td),this.title.actions.repeatForever(t=>{t.easeBy(z(0,-30*tp.y),1e3,f.EaseInOutQuad).easeBy(z(0,30*tp.y),1e3,f.EaseInOutQuad)}),this.add(this.title),this.instructions=new l({pos:z(400,600),coordPlane:g.Screen});let e=new x({family:"notjamslab14",size:32*tp.x,unit:w.Px,color:p.White,baseAlign:u.Top,quality:4,shadow:{offset:z(10,10).scale(tp),color:p.Black}}),i=new R({text:"Click to Play!",font:e});this.instructions.graphics.use(i),this.instructions.actions.repeatForever(t=>{// ctx.blink(300, 300, 2);
t.rotateTo(Math.PI/32,.2),t.rotateTo(-Math.PI/32,.2)}),this.add(this.instructions)}};eZ.addScene("start",eK);const eY=new class extends I{constructor(t){super(),this.levelData=t}onInitialize(t){this.engine=t,this.add(new eX(z(800,0))),this.add(new eX(z(400,300))),this.add(new eX(z(700,700))),this.board=this.parse(this.levelData),this.add(tC)}onActivate(){this.turnManager.start()}onDeactivate(){// TODO deactivate event handlers on types that have them!!
}parse(t){let e=new tb(t.height,t.width,this);this.selectionManager=new tI(e),this.uiManager=new eG(this.engine),// TODO support arbitrary players
this.players=[new tP(t.players[0],this.engine,this.selectionManager,this.uiManager,e),new eq(t.players[1],this.selectionManager,e)],this.turnManager=new eV(this.engine,this.players,this.selectionManager,10);for(let i=0;i<t.height;i++)for(let s=0;s<t.width;s++){let r=t.data[s+i*t.width],n=r.charAt(0),o=null;if(3===r.length){let t="K"===r.charAt(1)?"Knight":"Spider",n=+r.charAt(2)-1;o=new tT(s,i,t,e,this.players[n]),this.add(o)}let a=e.getCell(s,i);a&&(a.terrain=n,o&&a.addUnit(o))}return e}}({name:"Gentle Plains",width:6,height:6,maxTurns:10,players:["human","computer"],data:["GK1","G","G","G","G","GK1","G","G","G","G","G","G","G","G","W","W","G","G","G","G","W","W","G","G","G","G","W","W","G","G","GS2","GS2","GS2","GS2","GS2","GS2"]});eZ.addScene("level1",eY),eZ.start(tc).then(()=>{eZ.goToScene("start")});//# sourceMappingURL=index.2acf2ede.js.map

//# sourceMappingURL=index.2acf2ede.js.map
