...
return[]}}class to extends o{constructor(t,e,i,s,r){super({anchor:E(0,0)}),this.player=r,this.cell=null,this.moved=!1,this.unitConfig={...J[i]},this.anim=this.unitConfig.graphics.idle.clone(),this.anim.scale=j,this.graphics.use(this.anim),this.graphics.onPostDraw=this.onPostDraw.bind(this);let n=s.getCell(t,e);n&&(this.pos=n.pos.sub(this.unitConfig.graphics.offset),n.addUnit(this))}onPostUpdate(){this.moved?this.anim.tint=l.Gray:this.anim.tint=l.White}onPostDraw(t){let e=Z.getSprite(A(this.unitConfig.health,0,5),0);e&&(e.scale=j,e.draw(t,10*j.x,16*j.y))}async move(t){this.cell&&this.cell.removeUnit(this);let e=null;for(let i of t.slice(1,t.length)){e=i.owner;let t=new n(this,t=>{t.delay(200),t.callMethod(()=>{O.MoveSound.play()})}),s=new n(this,t=>{t.easeTo(e.pos.sub(this.unitConfig.graphics.offset),300,p.EaseInOutCubic)}),r=new v([t,s]);await this.actions.runAction(r).toPromise()}e&&e.addUnit(this),this.moved=!0}reset(){this.moved=!1}canMove(){return!this.moved}}class ta{static #t=this._STARTING_BIT=1;static #e=this._CURRENT_GROUP=ta._STARTING_BIT;constructor(t,e){this.name=t,this.board=e,this.active=!1,this.mask=ta._CURRENT_GROUP=ta._CURRENT_GROUP<<1|0}async turnStart(){this.active=!0;let t=this.board.getUnits().filter(t=>t.player===this);t.forEach(t=>t.reset())}async turnEnd(){this.active=!1}}const th=new _({width:800,height:800,displayMode:u.FitScreenAndFill});// TODO move to level
// Add clouds :3
...
this.selectionManager.currentUnitSelection?e?(this.active=!1,await this.selectionManager.selectDestinationAndMove(this.selectionManager.currentUnitSelection,e),this.humanMove.resolve()):this.selectionManager.reset():e?.unit&&e?.unit?.canMove()?this.selectionManager.selectUnit(e.unit):this.selectionManager.reset()}async waitForHumanMove(){this.active=!0,await this.humanMove.promise,this.humanMove=new g}hasMoves(){let t=this.board.getUnits().filter(t=>t.player===this).filter(t=>t.canMove());return t.length>0&&!this.passed}// TODO event handlers need to be here
async makeMove(){// TODO while there are moves to make or the player has not passed their turn wait
for(this.board.getUnits().filter(t=>t.player===this);this.hasMoves();)await this.waitForHumanMove();return!0}}("Knights of the Round Table",th,td,tl),tp=new class{constructor(t,e){if(this.players=t,this.currentPlayerIndex=0,0===t.length)throw Error("Players should be non-zero in length");this.currentPlayer=t[this.currentPlayerIndex],this.selectionManager=e}// todo generator??
async start(){let t=10;for(;t>0;){console.log("Current player turn:",this.currentPlayer.name),this.selectionManager.selectPlayer(this.currentPlayer),await this.currentPlayer.turnStart();let e=!0;do e=await this.currentPlayer.makeMove();while(!e)await this.currentPlayer.turnEnd(),this.nextTurn(),t--}}nextTurn(){this.currentPlayerIndex++,this.currentPlayer=this.players[this.currentPlayerIndex%this.players.length]}}([tu,tc],td),t_=new to(0,0,"Spider",tl,tc);th.add(t_);const tg=new to(4,5,"Spider",tl,tc);th.add(tg);const tm=new to(1,1,"Knight",tl,tu);th.add(tm);const tf=new to(2,1,"Knight",tl,tu);th.add(tf),th.start(K).then(()=>{tp.start()});//# sourceMappingURL=index.6cad6037.js.map

//# sourceMappingURL=index.6cad6037.js.map
